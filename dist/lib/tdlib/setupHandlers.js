...
function getSenderId(senderIdObj) {
...
export function setupClientHandlers(client, userId) {
    ...
    client.on('updateNewMessage', async (update) => {
        ...
        if (message.sender_id._ === 'messageSenderUser' && senderId !== BigInt(0)) {
            try {
                const tdUser = await client.getUser(Number(senderId));
                if (tdUser) {
        ...
        if (message.is_outgoing && message.sender_id._ === 'messageSenderUser' && message.chat_id > 0) {
            try {
                ...
                const tdRecipient = await client.getUser(recipientId);
                if (tdRecipient) {
        ...
        let chatTitle;
        try {
            const tdChat = await client.getChat(message.chat_id);
            if (tdChat) {
                ...
                if (tdChat.type._ === 'chatTypeSupergroup' && tdChat.type.supergroup_id && !tdChat.type.is_channel) {
                    try {
                        const supergroup = await client.getSupergroup(tdChat.type.supergroup_id);
                        if (supergroup && supergroup.is_forum) {
                            tdChat.is_forum = true;
                        }
                    }
                    catch (err) {
        ...
        const msgAnyForSendingState = message;
        ...
        const topicIdObj = msgAny.topic_id;
    ...
    client.on('updateDeleteMessages', async (update) => {
        ...
        const isPermanent = update.is_permanent;
        if (isPermanent) {
            wsManager.send(userId, {
    ...
    client.on('updateNewChat', async (update) => {
        ...
        if (chat.type._ === 'chatTypeSupergroup' && chat.type.supergroup_id && !chat.type.is_channel) {
            try {
                const supergroup = await client.getSupergroup(chat.type.supergroup_id);
                if (supergroup && supergroup.is_forum) {
                    isForum = true;
                    chatType = 'forum';
        ...
        if (chat.type._ === 'chatTypePrivate' && chat.type.user_id) {
            try {
                const tdUser = await client.getUser(chat.type.user_id);
                if (tdUser) {
                    await syncUser(tdUser);
...
//# sourceMappingURL=setupHandlers.js.map