...
export async function archiveMessage(message) {
    ...
    try {
        ...
        logger.info({ chatId: message.chat_id, messageId: message.id }, '[DEBUG] Message archived successfully');
        return {
            archived: true,
...
export async function markMessagesAsDeleted(chatId, messageIds) {
    try {
        ...
        logger.info({ chatId, messageIds }, 'Messages marked as deleted (content preserved)');
    }
    catch (error) {
        logger.error({ error, chatId, messageIds }, 'Failed to mark messages as deleted');
    }
}
export async function archiveMessageEdit(chatId, messageId, newContent) {
    try {
...
export function createMessageHandlers() {
    return {
        ...
        updateDeleteMessages: async (update) => {
        ...
        updateMessageContent: async (update) => {
            const chatId = update.chat_id;
