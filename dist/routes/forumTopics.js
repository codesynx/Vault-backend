...
export async function forumTopicRoutes(app) {
    ...
    app.get('/api/chats/:chatId/topics', async (request, reply) => {
        try {
            ...
            try {
                ...
                const tdTopics = await client.getForumTopics(Number(chatId));
                if (tdTopics.topics && tdTopics.topics.length > 0) {
                    ...
                    await Promise.all(tdTopics.topics.map((topic) => syncForumTopic(Number(chatId), topic)));
                }
            }
            catch (tdError) {
