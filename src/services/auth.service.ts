...
import type { TdUser } from '../lib/tdlib/types.js';

interface AuthSession {
  sessionId: string;
  phoneNumber: string;
  authStep: 'phone' | 'code' | 'password' | 'ready';
  codeInfo?: Record<string, unknown>;
}

const authSessions = new Map<string, AuthSession>();

export async function startPhoneAuth(phoneNumber: string): Promise<{ sessionId: string; codeInfo?: Record<string, unknown> }> {
  logger.info({ phoneNumber, timestamp: new Date().toISOString() }, '[DEBUG] Starting phone auth');

  let client: Awaited<ReturnType<typeof tdlibManager.createAuthClient>>['client'];
  ...
  try {
    ...
    logger.error({ phoneNumber, err: createErr, errMessage: (createErr as Error).message, errStack: (createErr as Error).stack }, '[DEBUG] Failed to create auth client');
    throw createErr;
  }
      ...
      try {
        ...
        if (state === 'authorizationStateWaitPhoneNumber' && !phoneSent) {
          ...
          try {
            ...
            const error = phoneErr as Error & { code?: number; _?: string };
            logger.error({
              sessionId,
          ...
          const codeInfo = codeStateDetails?.code_info as Record<string, unknown> | undefined;
          logger.info({
            sessionId,
...
export async function verifyCode(
  sessionId: string,
  code: string
): Promise<{ needsPassword: boolean; userId?: string; tokens?: { accessToken: string; refreshToken: string } }> {
  const session = authSessions.get(sessionId);
  if (!session) {
    throw new Error('Invalid session');
  }
  ...
  const newState = await new Promise<string>((resolve) => {
    const timeout = setTimeout(() => {
      client.off('updateAuthorizationState', onStateUpdate);
...
export async function verifyPassword(
  sessionId: string,
  password: string
): Promise<{ userId: string; tokens: { accessToken: string; refreshToken: string } }> {
  const session = authSessions.get(sessionId);
  if (!session) {
    throw new Error('Invalid session');
  }
...
async function finalizeAuth(
  ...
  const me = (await client.getMe()) as TdUser;
...
export async function getAuthStatus(sessionId: string): Promise<{ step: string; codeInfo?: Record<string, unknown> }> {
  const session = authSessions.get(sessionId);
  if (!session) {
    throw new Error('Invalid session');
  }
