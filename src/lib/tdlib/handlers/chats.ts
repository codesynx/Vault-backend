...
import type { TdChat, TdUpdate, TdChatPosition, TdMessage } from '../types.js';
import { invalidateChatsCache, invalidateChatCache } from '../../redis.js';

// Extract content preview from message (handles text and media)
function extractContentPreview(content: TdMessage['content']): string {
...
function getChatType(typeObj: TdChat['type']): string {
...
function getMainChatListOrder(positions?: TdChatPosition[]): string {
  if (!positions || positions.length === 0) {
    return '0';
  }
  // Find the position for the main chat list (chatListMain)
  const mainPosition = positions.find(
    (pos) => pos.list._ === 'chatListMain'
  );
  return mainPosition?.order || positions[0]?.order || '0';
}

async function countLocalUnread(chatId: number, lastReadInboxId: number): Promise<number> {
  try {
...
export async function syncChat(chat: TdChat, userId?: string): Promise<void> {
  try {
...
export function createChatHandlers(userId?: string) {
  return {
    updateNewChat: async (update: TdUpdate) => {
      const chat = update.chat as TdChat;
      await syncChat(chat, userId);
      await invalidateChatsCache(userId);
    },

    updateChatPosition: async (update: TdUpdate) => {
      const chatId = update.chat_id as number;
      const position = update.position as TdChatPosition;
    ...
    updateChatTitle: async (update: TdUpdate) => {
      const chatId = update.chat_id as number;
    ...
    updateChatLastMessage: async (update: TdUpdate) => {
      const chatId = update.chat_id as number;
    ...
    updateChatReadInbox: async (update: TdUpdate): Promise<number> => {
      const chatId = update.chat_id as number;
    ...
    updateChatReadOutbox: async (update: TdUpdate) => {
      const chatId = update.chat_id as number;
    ...
    updateChatIsForum: async (update: TdUpdate) => {
      const chatId = update.chat_id as number;
