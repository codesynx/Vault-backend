...
async function countLocalTopicUnread(chatId: number, topicId: number, lastReadInboxId: number): Promise<number> {
  try {
...
export async function syncForumTopic(chatId: number, topic: TdForumTopic): Promise<number> {
  try {
    ...
    logger.debug(
      { chatId, topicId, name: info.name, localUnreadCount },
      'Forum topic synced'
    );
...
export function createForumTopicHandlers(client: TdlibClient) {
  return {
    // Called when forum topics are loaded for a chat
    updateForumTopicInfo: async (update: TdUpdate) => {
    ...
    updateChatActiveStories: async (_update: TdUpdate) => {
      // Not relevant for forums, skip
    },

    // Called when a topic is closed/reopened
    updateForumTopicClosed: async (update: TdUpdate) => {
      const chatId = update.chat_id as number;
    ...
    updateForumTopicHidden: async (update: TdUpdate) => {
      const chatId = update.chat_id as number;
      const messageThreadId = update.message_thread_id as number;
    ...
    updateForumTopicUnreadCount: async (update: TdUpdate): Promise<number> => {
      const chatId = update.chat_id as number;
      const messageThreadId = update.message_thread_id as number;
